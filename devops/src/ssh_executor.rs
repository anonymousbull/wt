...
impl SshExecutor {
    ...
    pub async fn copy_file_to_remote(&self, local_path: &str, remote_path: &str) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            let mut remote_file = session.scp_send(Path::new(&remote_path), 0o644, 0, None)
                .with_context(|| format!("Failed to open remote file: {}", remote_path))?;
            let mut local_file = std::fs::File::open(&local_path)
                .with_context(|| format!("Failed to open local file: {}", local_path))?;
            let mut buffer = Vec::new();
            local_file.read_to_end(&mut buffer).context("Failed to read local file")?;
            remote_file.write_all(&buffer).map_err(|e| anyhow::anyhow!("Failed to write to remote file: {}", e))?;
            Ok(())
        }).await.context("Failed to run blocking task")?
    }
